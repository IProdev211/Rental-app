...
import { deleteUsers } from '../../redux/reducers/userReducer';
import { useAlertMessage } from '../../hooks/useAlertMessage';
import EditUserModal from './modals/EditUser';

type Data = TUser & { action?: string }
type Order = 'asc' | 'desc';
interface HeadCell {
...
export default function UserTable({ users }: { users: TUser[] }) {
  ...
  const [openAddNewUserModal, setOpenAddNewUserModal] = React.useState<boolean>(false)
  const [openEditUserModal, setOpenEditUserModal] = React.useState<boolean>(false)
  const [selectedUser, setSelectedUser] = React.useState<TUser | undefined>()
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelecteds = users.map((n) => n.id as string);
  ...
  const handleOpenEditUser = (user: TUser) => {
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%' }}>
        <EnhancedTableToolbar numSelected={selected.length} onClickAdd={handleOpenAddUser} onClickDelete={handleDeleteUsers} />
        <TableContainer>
          <Table
            sx={{ minWidth: 750 }}
                  ...
                  .map((row, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>
                          <Tooltip title="Edit a user">
                            <IconButton onClick={() => handleOpenEditUser(row)}>
                              <EditIcon />
                            </IconButton>
                          </Tooltip></TableCell>
                      </TableRow>
      ...
      <EditUserModal
        ...
        open={Boolean(selectedUser) && openEditUserModal}
